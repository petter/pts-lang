...
function setScope(node, scope) {
    return __assign(__assign({}, node), { children: node.children.map(function (c) { return setScope(c, scope); }), scope: scope });
}
function toScopedAST(program) {
    ...
    var scopedAst = index_1.default(rootScopedAst, function (revisit) {
        var makeScope = function (node, _) {
            ...
            var scopedChildren = revisit(node.children.map(function (child) { return setScope(child, scope); }));
        ...
        return {
            ...
            default: function (node, children) { return (__assign(__assign({}, node), { children: children })); }
        };
    });
